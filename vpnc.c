...
static struct sa_block *atexit_sa;
...
/* Return value should be free()'d if not NULL */
static char * ask_answer_script(char *message, int count) {
	int fd[2];
	pid_t child;

	if (socketpair(AF_UNIX, SOCK_STREAM, 0, fd) == -1) {
		error(0, errno, "ask_answer_script: socketpair()");
		return NULL;
	}
	child = fork();
	if (child == -1) {
		error(0, errno, "ask_answer_script: fork()");
		return NULL;
	} else if (child == 0) {
		/* Child */
		close(fd[0]);
	   	if (dup2(fd[1], STDIN_FILENO) == -1) {
			error(1, errno, "ask_answer_script: dup2(STDIN_FILENO)");
	   	}
	   	if (dup2(fd[1], STDOUT_FILENO) == -1) {
			error(1, errno, "ask_answer_script: dup2(STDOUT_FILENO)");
	   	}
        char *cmd;
        asprintf(&cmd, "%s %d", config[CONFIG_XAUTH_ANSWER_SCRIPT], count);
		int ret = system(cmd);
        free(cmd);
        char end[2];
        end[0] = '\0';
        ...
        char *p = end;
        int c;
        while (p < (end + 2) && (c = write(fd[1], p, (end + 2) - p)) >= 0) {
            p += c;
        }
        close(fd[1]);
		exit(0);
		...
		/* Parent */
		char buf[512], *answer = NULL;
		int nr;
		int len = strlen(message) + 1;
		close(fd[1]);
		while (len) {
			int w;
			if ((w = write(fd[0], message, len)) == -1) {
				error(0, errno, "ask_answer_script: write()");
				close(fd[0]);
				return NULL;
			}
			len -= w;
		...
		if (shutdown(fd[0], SHUT_WR) == -1) {
			error(0, errno, "ask_answer_script: shutdown()");
			close(fd[0]);
			return NULL;
		}
		len = 0;
		while ((nr = read(fd[0], buf, sizeof(buf))) > 0) {
			if (answer == NULL) {
				if ((answer = (char *)malloc(len + nr)) == NULL)
					error(1, errno, "ask_answer_script: malloc()");
			} else {
				if ((answer = (char *)realloc(answer, len + nr)) == NULL)
					error(1, errno, "ask_answer_script: realloc()");
			}
			memcpy(answer + len, buf, nr);
			len += nr;
		}
		close(fd[0]);
		
		if (nr < 0) {
            error(0, errno, "ask_answer_script: read()");
            if (answer != NULL) 
                free(answer);
            return NULL;
		}

        // Check last char. if 0, return value, else return NULL.
		// Make sure last char is '\0'
		if (answer != NULL && len > 0 && *(answer + len - 1) == 0) {
            return answer;
		} else {
		    if (answer != NULL)
		        free(answer);
...
static int do_phase2_xauth(struct sa_block *s)
{
	...
	for (loopcount = 0;; loopcount++) {
		...
		char ntop_buf[32];
		int seen_answer = 0, seen_passcode = 0, seen_user = 0, seen_user_password = 0, seen_message = 0;
		int answer_workaround = 0;
		...
		for (ap = a; ap && seen_answer == 0; ap = ap->next)
			if (ap->type == ISAKMP_XAUTH_06_ATTRIB_ANSWER)
				seen_answer = 1;
			else if (ap->type == ISAKMP_XAUTH_06_ATTRIB_MESSAGE)
				seen_message = 1;
			else if (ap->type == ISAKMP_XAUTH_06_ATTRIB_NEXT_PIN
	    		 || ap->type == ISAKMP_XAUTH_06_ATTRIB_PASSCODE)
				seen_passcode = 1;
			else if (ap->type == ISAKMP_XAUTH_06_ATTRIB_USER_PASSWORD)
				seen_user_password = 1;
			else if(ap->type == ISAKMP_XAUTH_06_ATTRIB_USER_NAME)
				seen_user = 1;
				
		if (config[CONFIG_ANSWER_AS_PASSWORD_WORKAROUND] && seen_user_password && seen_message && !seen_user) 
			/* If we've seen a password request with no user request, and we have the workaround
			 * enabled, turn the password request into an answer request.
			 */
		{
			answer_workaround = 1;
			seen_answer = 1;
			DEBUG(2, printf("Answer workaround enabled\n"));
		}			
		...
		for (ap = a; ap && reject == 0; ap = ap->next)
			...
			case ISAKMP_XAUTH_06_ATTRIB_MESSAGE:
				if (opt_debug || seen_answer || seen_passcode || config[CONFIG_XAUTH_INTERACTIVE]) {
                    if (seen_answer && config[CONFIG_XAUTH_ANSWER_SCRIPT] && ap->af != isakmp_attr_16) {
                    	if (last_question != NULL) {
                            free(last_question);
                            last_question = NULL;
                    	}
                        if (asprintf(&last_question, "%.*s", ap->u.lots.length, ap->u.lots.data) == -1) {
                            error(1, errno, "asprintf(last_question)");
                        }
                    }
		...
		for (ap = a; ap && reject == 0; ap = ap->next) 
		{
			...
			case ISAKMP_XAUTH_06_ATTRIB_ANSWER:
answer_workaround:
				answer_workaround_used = 1;
				/* Try the answer script first */
				if (config[CONFIG_XAUTH_ANSWER_SCRIPT] && last_question != NULL) {
					char *answer = ask_answer_script(last_question, question_count++);
					if (answer != NULL) {
						struct isakmp_attribute *na;
						DEBUG(2, printf("Sending script-generated answer\n"));
                        printf("Script response: %s\n", answer);
						na = new_isakmp_attribute(ap->type, reply_attr);
						reply_attr = na;
						na->u.lots.length = strlen(answer);
						na->u.lots.data = xallocc(na->u.lots.length);
						memcpy(na->u.lots.data, answer, na->u.lots.length);
						free(answer);
						break; /* switch */
					} else {
						DEBUG(2, printf("Answer script had no response\n"));
					}
				} 
				/* no break */
			case ISAKMP_XAUTH_06_ATTRIB_USER_PASSWORD:
			if (answer_workaround && !answer_workaround_used) {
				DEBUG(2, printf("goto answer_workaround\n"));
				goto answer_workaround;
			}
			case ISAKMP_XAUTH_06_ATTRIB_PASSCODE:
			case ISAKMP_XAUTH_06_ATTRIB_NEXT_PIN:
				...
				} else if (seen_answer || seen_passcode || passwd_used || config[CONFIG_XAUTH_INTERACTIVE]) {
					char *pass, *prompt = NULL;
					struct isakmp_attribute *na;

					asprintf(&prompt, "%s for VPN %s@%s: ",
						(ap->type == ISAKMP_XAUTH_06_ATTRIB_ANSWER || answer_workaround) ?
						"Answer" :
...
static void close_and_cleanup(struct sa_block *s) {
    if (s->should_close_and_cleanup) {
    	/* Cleanup routing */
        ...
        s->should_close_and_cleanup = 0;
    }
}

static void atexit_func() {
	if (atexit_sa != NULL) {
	    close_and_cleanup(atexit_sa);
	}
}
...
int main(int argc, char **argv)
{
	...
	memset(s, 0, sizeof(struct sa_block));
	s->ipsec.encap_mode = IPSEC_ENCAP_TUNNEL;
